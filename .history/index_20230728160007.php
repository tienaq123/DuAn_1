...
<body>
  ...
  <script>
    ...
    $("#btnLoadMore").on("click", function() {
      ...
      $.ajax({
        type: "GET",
        url: "load_more_products.php", // Đường dẫn đến tập tin PHP xử lý truy vấn CSDL
        data: {
  ...
  Tạo tập tin PHP (ví dụ: load_more_products.php) để xử lý truy vấn CSDL và trả về danh sách sản phẩm mới:
  php
  Copy code
  <?php
  // Lấy số trang tiếp theo từ tham số GET
  ...
  // Thực hiện truy vấn CSDL và lặp qua kết quả để hiển thị sản phẩm mới
  $result_products = mysqli_query($conn, $query_products);
  if (mysqli_num_rows($result_products) > 0) {
    while ($row = mysqli_fetch_assoc($result_products)) {
      // Hiển thị sản phẩm tương tự như phần mã PHP hiển thị ở trang chính
      // Bạn có thể sử dụng vòng lặp để lặp qua các sản phẩm và hiển thị chúng
    }
  }
  ?>
  Trong tập tin PHP load_more_products.php, bạn thực hiện truy vấn CSDL để lấy danh sách sản phẩm tiếp theo dựa trên số trang được truyền qua tham số GET. Sau đó, bạn sẽ lặp qua kết quả và hiển thị các sản phẩm mới bằng cách sử dụng mã PHP tương tự như phần hiển thị sản phẩm ở trang chính.

  Lưu ý rằng, đoạn mã JavaScript sẽ gọi tới tập tin load_more_products.php thông qua Ajax và sau đó thêm danh sách sản phẩm mới vào phần container (<div id="containerisotope">) sử dụng phương th

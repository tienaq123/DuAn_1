...
<body>
    ...
    <div class="container clearfix">
      ...
      <div id="containerisotope" class="clear">
        ...
        <?php
          ...
          // Sử dụng một mảng để lưu danh sách ID sản phẩm trong trang hiện tại
          $current_product_ids = array();
          while ($row = mysqli_fetch_assoc($result_products)) {
            $current_product_ids[] = $row['id'];
          }

          // Kiểm tra xem ID của sản phẩm tiếp theo có trong mảng ID của trang hiện tại không
          $has_next_page = in_array(mysqli_fetch_assoc($result_next_page)['id'], $current_product_ids);
        } else {
  ...
  <!-- My word -->
  <script>
    const container = document.getElementById('containerisotope');
    const btnNext = document.getElementById('btnNext');

    let slideIndex = 0;
    const productsPerPage = 8;
    const productWidth = 250; // Độ rộng của mỗi sản phẩm

    btnNext.addEventListener('click', () => {
      slideIndex++;
      const translateXValue = -slideIndex * (productsPerPage * productWidth);

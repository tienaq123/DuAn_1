...
<body>
        ...
        <nav class="rightnavigation">
          ...
          "><div class="dropdown">
              ...
              <div class="dropdown-content">
                <a href="./Pages/order/order-list.php">Order</a>
                <a href="./Pages/profile/profile.php">Changer</a>
        ...
        ?>
            ...
            <div style="min-height: 420px; border: 1px solid #eaeaea;" class="element <?php echo str_replace(' ', '', $row['category_name']) ?> " data-category="<?php echo str_replace(' ', '', $row['category_name']) ?>">
              ...
              <a data-rel="prettyPhoto[]" href="<?php echo $row['thumbnail'] ?>">
                <img style="margin-top: 0;" alt="" class="imgwork" src="<?= $row['thumbnail'] ?>" />
              </a>
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Lấy thông tin sản phẩm từ form hoặc từ CSDL
    ...
    if (isset($_SESSION['user_id'])) {
      // Nếu đã đăng nhập, kiểm tra xem giỏ hàng của "khách" có sản phẩm hay không
      if (isset($_SESSION['guest_cart'])) {
        // Sao chép sản phẩm từ giỏ hàng của "khách" vào giỏ hàng của người dùng đã đăng nhập
        foreach ($_SESSION['guest_cart'] as $guest_item) {
          $guest_product_id = $guest_item['id'];
          $guest_product_quantity = $guest_item['quantity'];

          $user_cart_exists = false;
          foreach ($_SESSION['cart'] as $user_key => $user_item) {
            if ($user_item['id'] == $guest_product_id) {
              // Sản phẩm đã có trong giỏ hàng của người dùng, tăng số lượng lên
              $_SESSION['cart'][$user_key]['quantity'] += $guest_product_quantity;
              $user_cart_exists = true;
              break;
            }
          }
  ...
  <!-- Back to top -->
  <script>
    // Hàm xử lý khi người dùng click nút Back to Top
    function topFunction() {
      document.documentElement.scrollTop = 0;
    }

    // Hiển thị hoặc ẩn nút Back to Top dựa trên vị trí của người dùng
  ...
  <a href="#" id="backToTopBtn" class="back-to-top"><i class='bx bxs-up-arrow'></i></a>
  <!-- <script>

    // Nếu người dùng cuộn xuống hơn 20px từ đỉnh trang, hiển thị nút Back to Top
    window.onscroll = function() {
      scrollFunction();
    };
    ...
    // Xử lý sự kiện khi người dùng click vào nút Back to Top

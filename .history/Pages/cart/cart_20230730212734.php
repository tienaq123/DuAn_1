...
<body>
                  ...
                  <div class="cart_products-text">
                    ...
                    <div class="text-quantity">
                      <div style="display: flex">
                        <button style="width: 30px; height: 30px; border: none; cursor: pointer;" onclick="updateQuantity('<?php echo $item['id']; ?>', <?php echo $item['quantity'] - 1; ?>)">
                          -
                        </button>
                        <input style="
                          width: 30px;
                          ...
                          text-align: center;
                          border: none;
                        " type="number" class="no-spinners" value="<?php echo $item['quantity'] ?>" readonly />
  ...
  <script>
    ...
    increaseButtons.forEach((button) => {
      button.addEventListener('click', function() {
        ...
        // Cập nhật giá trị quantity trong session
    ...
    // Hàm cập nhật giá trị quantity trong session
    function updateQuantityInCart(productID, newQuantity) {
      // Gửi yêu cầu Ajax để cập nhật quantity trong session
      const xhr = new XMLHttpRequest();
      xhr.open('POST', 'update_quantity_in_cart.php', true);
      ...
      const data = 'product_id=' + productID + '&quantity=' + newQuantity;
      xhr.send(data);
    }
  </script>
  ...
  <script>
    function updateQuantity(productId, quantityChange) {
      ...
      var newQuantity = parseInt(document.getElementById('quantity-' + productId).value) + quantityChange;
      ...
      xhr.onreadystatechange = function() {
        if (xhr.readyState === XMLHttpRequest.DONE) {
          if (xhr.status === 200) {
            // Xử lý kết quả trả về nếu cần thiết
            // Thực hiện các thay đổi trong giao diện nếu cần
            console.log(xhr.responseText);
            // Reload trang để cập nhật lại giỏ hàng (nếu cần)
            location.reload();
          } else {
            // Xử lý lỗi nếu có
            console.error('Đã xảy ra lỗi: ' + xhr.status);

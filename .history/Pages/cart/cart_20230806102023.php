...
<body>
  ...
  <main id="main">
    <div class="container">
      ...
      <p>Có <?php echo $totalProductsInCart ?> sản phẩm trong giỏ hàng</p>
      <div class="line"></div>
      <div class="cart_content">
        ...
        <div class="cart_products">
          ...
          if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'remove') {
            ...
            echo 'success'; //gửi dữ liệu mới về
            exit;
          }
          // Kiểm tra nếu giỏ hàng không tồn tại hoặc rỗng
          if (!isset($_SESSION['cart']) || empty($_SESSION['cart'])) {
            echo '<h2 class="echo">Giỏ hàng của bạn đang trống.</h2>';
            echo '<a style="text-align: center; color: black; font-weight: bold" class="cart_bill-back" href="../../index.php#sectionportfolio">
            <i class="bx bx-reply"></i>
            <span>Thêm sản phẩm vào giỏ</span>
          </a>';
          } else {
            // Hiển thị thông tin các sản phẩm trong giỏ hàng
            $totalAmount = 0; // Khởi tạo biến lưu tổng tiền

            foreach ($_SESSION['cart'] as $item) {
              $productTotal = $item['price'] * $item['quantity'];
              $totalAmount += $productTotal; // Cộng tổng tiền của sản phẩm này vào tổng tiền chung
                  ...
                  <div class="cart_products-text">
                    ...
                    <div class="text-quantity">
                      <div style="display: flex">
                        <button style="width: 30px; height: 30px; border: none; cursor: pointer;">
                          -
                        </button>
                        <input id="quantityInput-<?php echo $item['id']; ?>" style="
                          ...
                          text-align: center;
                          border: none;
                        " type="number" class="no-spinners" value="<?php echo  intval($item['quantity']) ?>" onchange="updateCartQuantity('<?php echo  intval($item['id']); ?>', this.value)" data-productId="<?php echo $item['id']; ?>" readonly />
                        <button style="width: 30px; height: 30px; border: none; cursor: pointer;">
                          +
                        </button>
        ...
        <div class="cart_bill">
          ...
          <div class="cart_bill-price">
            <span>Tổng tiền:</span> <span><?php echo number_format($totalAmount); ?>₫</span>
          </div>
          ...
          <form method="post" action="checkout.php">
            <div class="cart_bill-btn">
              <!-- Sử dụng button để submit form -->
              <button type="submit" class="codepro-custom-btn codepro-btn-15">
                Thanh toán
              </button>
            </div>
          </form>
      ...
      <?php $_SESSION['cart_total'] = $totalAmount;
          }
      ?>
      <!-- end cart_bill -->
      </div>

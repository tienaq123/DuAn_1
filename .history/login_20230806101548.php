  ...
  if (mysqli_num_rows($result_login) == 1) {
    ...
    if (password_verify($password, $hashed_password)) {
      ...
      if (isset($_SESSION['redirect_url'])) {
        // Chuyển thông tin giỏ hàng từ session hiện tại sang session của người dùng đăng nhập
        $_SESSION['user_cart'] = $_SESSION['cart'];
        // Xóa session $_SESSION['redirect_url'] để không thực hiện chuyển thông tin giỏ hàng lần nữa

      ...
      <div class="body-content px-8 py-8 bg-slate-100">
        ...
        if (isset($_POST['add_category'])) {
          ...
          if (mysqli_query($conn, $sql)) {
            ...
            // header("Location: Duan1/Admin/category.php");
          } else {
            ...
            <div class="mb-6 bg-white px-8 py-8 rounded-md">
              ...
              <form method="post">
                ...
                <div class="mb-6">
                  ...
                  <input class="input w-full h-[44px] rounded-md border border-gray6 px-6 text-base" type="text" name="category_name" placeholder="Name" require />
                </div>
                          ...
                          <tr class="bg-white border-b border-gray6 last:border-0 text-start mx-9">
                            ...
                            <td class="px-9 py-3 text-end">
                              <div class="flex items-center justify-end space-x-2">
                                ...
                                <?php
                                // Kiểm tra xem người dùng đã nhấn nút "Delete" và gửi CSRF token hay chưa
                                if (isset($_POST['delete_category']) && isset($_POST['csrf_token']) && $_POST['csrf_token'] === $_SESSION['csrf_token']) {
                                  // Trích xuất ID của danh mục cần xóa từ form
                                  $category_id = $_POST['category_id'];

                                  // Kiểm tra và xác thực dữ liệu đầu vào
                                  // (ví dụ: kiểm tra xem $category_id có phải là số nguyên không âm hợp lệ, nếu cần)
                                  ...
                                  // Thực thi truy vấn
                                  if (mysqli_query($conn, $sql)) {
                                    // Nếu xóa thành công, bạn có thể thêm mã xử lý để hiển thị thông báo hoặc chuyển hướng người dùng
                                    // echo "Xóa danh mục thành công!";
                                    // Hoặc chuyển hướng về trang danh mục sau khi xóa thành công
                                    header("Location: http://localhost/Duan1/Admin/category.php");
                                    exit;
                                  } else {
                                    // Nếu có lỗi xảy ra trong quá trình xóa, bạn có thể hiển thị thông báo hoặc xử lý lỗi ở đây
                                    echo "Lỗi: " . mysqli_error($conn);
                                  }
                                } else {
                                  // Nếu không tồn tại CSRF token hoặc không trùng khớp, bạn có thể hiển thị thông báo hoặc xử lý lỗi ở đây
                                  echo "Lỗi xác thực CSRF!";
                                }
                                ?>

                                <form method="post" action="">
                                  <input type="hidden" name="category_id" value="<?php echo $row['id']; ?> <?php echo $csrf_token; ?>">

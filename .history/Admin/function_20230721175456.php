...

{
    ...
    // Kiểm tra xem phần tử "product_name" có tồn tại trong mảng $productData hay không
    $title = isset($productData['product_name']) ? $productData['product_name'] : '';

    // Kiểm tra xem phần tử "description" có tồn tại trong mảng $productData hay không
    $description = isset($productData['description']) ? $productData['description'] : '';

    // Kiểm tra xem phần tử "price" có tồn tại trong mảng $productData hay không
    $price = isset($productData['price']) ? $productData['price'] : '';

    // Kiểm tra xem phần tử "old_price" có tồn tại trong mảng $productData hay không
    $old_price = isset($productData['old_price']) ? $productData['old_price'] : '';

    // Kiểm tra xem phần tử "category" có tồn tại trong mảng $productData hay không
    $category_id = isset($productData['category']) ? $productData['category'] : '';

    // Thêm trường created_at vào dữ liệu post
    $created_at = date('Y-m-d H:i:s'); // Lấy thời gian hiện tại

    // Chuẩn bị truy vấn SQL để thêm sản phẩm vào bảng Product
    $sql = "INSERT INTO Product (title, description, price, old_price, category_id, created_at)
            VALUES ('$title', '$description', $price, $old_price, $category_id, '$created_at')";

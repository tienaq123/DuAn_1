...
if (isset($_GET['keyword'])) {
  ...
  $sql = "SELECT o.*, od.* FROM orders o
        ...
        WHERE o.order_id = '$keyword'"; // Đây là điều kiện tìm kiếm theo ID
  ...
  if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
      ...
      echo '<div>';
      echo '<img style="margin-right: 18px;" class="w-[60px] h-[60px] rounded-md" src="' . $row['fullname'] . '" alt="" />';
      echo '<span class="font-medium text-heading text-hover-primary transition">' . $row['email'] . '</span>';
      echo '</div>';

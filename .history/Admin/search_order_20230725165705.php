...
if (isset($_GET['keyword'])) {
  ...
  if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
      ...
      echo '<div class="bg-white border-b border-gray6 last:border-0 text-start mx-9">';
      echo '<span class="font-medium text-heading text-hover-primary transition pr-8 py-5 whitespace-nowrap">#' . $row['id'] . '</span>';
      echo '<span class="font-medium text-heading text-hover-primary transition pr-8 py-5 whitespace-nowrap">' . $row['fullname'] . '</span>';
      echo '<span class="font-medium text-heading text-hover-primary transition pr-8 py-5 whitespace-nowrap">' . $row['total_money'] . '</span>';
      echo '<span class="font-medium text-heading text-hover-primary transition pr-8 py-5 whitespace-nowrap">' . $row['status'] . '</span>';
      echo '</div>';
      echo '<div style="margin-right: 40px;" class="flex items-center justify-end space-x-2">';
    ...
    echo "Không có đơn hàng nào phù hợp.";
  } ?>
